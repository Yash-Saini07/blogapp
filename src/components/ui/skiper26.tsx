...
// Add global type definition for View Transitions API
declare global {
    interface Document {
...
const OptionBtn: React.FC<OptionBtnProps> = ({ label, isActive, onClick }) => (
    <button
        onClick={onClick}
        className={cn(
            ...
            <div className="flex flex-col">
                <div className="mt-1 flex justify-between py-1">
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-1">
                        {["circle", "rectangle", "gif", "polygon", "circle-blur"].map(
                            (v) => (
                                <OptionBtn
                                    key={v}
                                    label={v}
                    ...
                    variant === "circle-blur") && (
                        <div className="mt-1 flex justify-between py-1">
                            ...
                            <div className="flex flex-wrap items-center justify-end gap-1">
                                ...
                                {variant === "rectangle" && (
                                    <>
                                        {["bottom-up", "top-down", "left-right", "right-left"].map(
                                            (dir) => (
                                                <OptionBtn
                                                    key={dir}
                                                    label={dir}
                                ...
                                {(variant === "circle" ||
                                    ...
                                    variant === "circle-blur") && (
                                        <>
                                            <OptionBtn
                                                label="top-left"
                                                isActive={start === "top-left"}
                                                onClick={() => setStart("top-left")}
                                            />
                                            <OptionBtn
                                                label="top-right"
                                                isActive={start === "top-right"}
                                            ...
                                            {variant !== "polygon" && (
                                                <>
                                                    <OptionBtn
                                                        label="bottom-left"
                                                        isActive={start === "bottom-left"}
                                                        onClick={() => setStart("bottom-left")}
                                                    />
                                                    <OptionBtn
                                                        label="bottom-right"
                                                        isActive={start === "bottom-right"}
                                ...
                                {(variant === "circle" || variant === "circle-blur") && (
                                    <>
                                        <OptionBtn
                                            label="top-center"
                                            isActive={start === "top-center"}
                                            onClick={() => setStart("top-center")}
                                        />
                                        <OptionBtn
                                            label="bottom-center"
                                            isActive={start === "bottom-center"}
...
export const useThemeToggle = ({
    ...
    const applyThemeWithAnimation = useCallback(
        ...
        [variant, start, blur, gifUrl, updateStyles, setTheme],
    );

    const toggleTheme = useCallback(() => {
        const newTheme = theme === "light" ? "dark" : "light";
        applyThemeWithAnimation(newTheme, !isDark);
    }, [theme, isDark, applyThemeWithAnimation]);

    const setCrazyLightTheme = useCallback(() => {
        applyThemeWithAnimation("light", false);
    }, [applyThemeWithAnimation]);

    const setCrazyDarkTheme = useCallback(() => {
        applyThemeWithAnimation("dark", true);
    }, [applyThemeWithAnimation]);
    ...
    const setCrazySystemTheme = useCallback(() => {
        ...
        applyThemeWithAnimation("system", prefersDark);
    }, [applyThemeWithAnimation]);
